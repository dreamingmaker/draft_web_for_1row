...
- `어절 줄나눔`: 선택 문단의 한글 줄 나눔 기준을 어절로 바꿔, 띄어쓰기 없는 단어가 줄 사이에서 쪼개지는 일을 줄입니다.
- `4칸 단축`, `2칸 늘리기`를 실행할 때도 먼저 `어절 줄나눔`을 자동 적용합니다.
...
4. 텍스트를 드래그한 뒤 오른쪽 아래의 `4칸 단축`, `2칸 늘리기`, `어절 줄나눔` 버튼을 누릅니다.
...
applyWordBreakBeforeSpacing: true, // 자간 조정 전 어절 줄나눔 자동 적용
```
