...
- `4칸 단축`: 선택한 텍스트에 HWP `CharShapeSpacingDecrease` 액션을 4회 실행합니다.
- `2칸 늘리기`: 선택한 텍스트에 HWP `CharShapeSpacingIncrease` 액션을 2회 실행합니다.
- 웹 기안기가 iframe 안에 있어도 접근 가능한 HWP 컨트롤 객체를 찾아 실행합니다.
- 실행 결과와 대상 정보는 콘솔 로그에 남기고, 화면에는 상태만 간단히 표시합니다.
